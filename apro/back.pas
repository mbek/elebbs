...
program Back;
  {-Demonstration of TSR background protocol}
uses
  {--- RTL  ---}
  Dos,
  {--- OPRO ---}
...
const
  ...
  ClockHandle  : Byte = 15;
  ModuleName   : String[5]  = 'BACK';
  PgmId        : String[10] = 'Back 2.03';
...
var
  ...
  ComPort        : PortRecPtr;              {ComPort record}
  PP             : ProtocolRecPtr;          {Protocol record}
...
begin
  InitPort(ComPort, ComName, Baud, NoParity, 8, 1, 2148, 2148, DefPortOptions);
  ptOptionsOff(ComPort, ptDropModemOnClose);
  if AsyncStatus <> ecOk then begin
    PopMsg('Failed to open port, error was '+Long2Str(AsyncStatus));
    ProtocolInProg := False;
    Exit;
  end;

  {Set a port abort function}
  SetAbortFunc(ComPort, ResidentAbort);

  {Set user hooks}
  SetErrorProc(ComPort, XErrorProc);
...
var
  ...
  begin
    ...
    if PP <> nil then
      {$IFDEF DemoXYModem}
      case PInfo^.Prot of
        Xmodem : DoneXmodem(PP);
        Ymodem : DoneYmodem(PP);
      end;
      {$ENDIF}
      {$IFDEF DemoZmodem}
      DoneZmodem(PP);
    ...
    if Error then
      ptOptionsOff(ComPort, ptRestoreOnClose+ptDropModemOnClose);
...
begin
  ...
  if PFirst then begin
    ...
    {$IFDEF DemoXYModem}
    case PInfo^.Prot of
      Xmodem : InitXmodem(PP, ComPort, PInfo^.Use1KBlocks, PInfo^.UseGMode);
      Ymodem : InitYmodem(PP, ComPort, True, PInfo^.UseGMode);
    end;
    {$ENDIF}
    {$IFDEF DemoZmodem}
   ...
   {More inits}
    ...
    case PInfo^.TransferMode of
      tmTransmit :
        begin
          ...
          {$IFDEF DemoXYModem}
          case PInfo^.Prot of
            Xmodem : PrepareTransmitPartXM(PP);
            Ymodem : PrepareTransmitPartYM(PP);
          end;
          {$ENDIF}
          {$IFDEF DemoZmodem}
          PrepareTransmitPartZM(PP);
          {$ENDIF}
          {$IFDEF DemoKermit}
          PrepareTransmitPartKM(PP);
          {$ENDIF}
          {$IFDEF DemoBPlus}
          PrepareTransmitPartBP(PP);
          {$ENDIF}
          if AsyncStatus <> ecOk then begin
            StopProtocol(False);
            Exit;
      ...
      tmReceive :
        begin
          SetOverwriteOption(PP, WriteRename);
          {$IFDEF DemoXYModem}
          SetReceiveFilename(PP, PInfo^.FName);
          case PInfo^.Prot of
            Xmodem : PrepareReceivePartXM(PP);
            Ymodem : PrepareReceivePartYM(PP);
          end;
          {$ENDIF}
          {$IFDEF DemoZmodem}
          PrepareReceivePartZM(PP);
          {$ENDIF}
          {$IFDEF DemoKermit}
          PrepareReceivePartKM(PP);
          {$ENDIF}
          {$IFDEF DemoBPlus}
          PrepareReceivePartBP(PP);
          {$ENDIF}
          if AsyncStatus <> ecOk then begin
            StopProtocol(False);
            Exit;
    ...
    GetIntVec(ComPort^.IrqNumber+8, OurVec);
  end;

  {Protocol is in progress - assure we still own the vector}
  GetIntVec(ComPort^.IrqNumber+8, CurVec);
  ...
  case PInfo^.TransferMode of
    tmTransmit :
      begin
        {$IFDEF DemoXYModem}
        case PInfo^.Prot of
          Xmodem : State := ProtocolTransmitPartXM(PP);
          Ymodem : State := ProtocolTransmitPartYM(PP);
        end;
        {$ENDIF}
        {$IFDEF DemoZmodem}
        State := ProtocolTransmitPartZM(PP);
        {$ENDIF}
        {$IFDEF DemoKermit}
        State := ProtocolTransmitPartKM(PP);
        {$ENDIF}
        {$IFDEF DemoBPlus}
        State := ProtocolTransmitPartBP(PP);
        {$ENDIF}
        {Process results}
        case State of
          psFinished :
    ...
    tmReceive :
      begin
        {$IFDEF DemoXYModem}
        case PInfo^.Prot of
          Xmodem : State := ProtocolReceivePartXM(PP);
          Ymodem : State := ProtocolReceivePartYM(PP);
        end;
        {$ENDIF}
        {$IFDEF DemoZmodem}
        State := ProtocolReceivePartZM(PP);
        {$ENDIF}
        {$IFDEF DemoKermit}
        State := ProtocolReceivePartKM(PP);
        {$ENDIF}
        {$IFDEF DemoBPlus}
        State := ProtocolReceivePartBP(PP);
        {$ENDIF}
        {Process results}
        case State of
          psFinished :
...
begin
  ...
  case C of
    ...
    'R' : if not ProtocolInProg then begin
            ...
            {$IFDEF DemoXYModem}
            if GetProtocolType then begin
              if ((PInfo^.Prot = Xmodem) and GetFile) or               {!!.03}
                 (PInfo^.Prot = Ymodem) then begin                     {!!.03}
            {$ENDIF}
                PFirst := True;
                ProtocolInProg := True;
                PInfo^.TransferMode := tmReceive;
  ...
  with PP^ do begin
    ...
    repeat
      if TimerExpired(RepeatTimer) then begin
        ...
        {$IFDEF DemoXYModem}
        State := Ord(XmodemPtr(PP)^.XmodemState);
        {$ENDIF}
        {$IFDEF DemoZmodem}
        State := Ord(ZmodemPtr(PP)^.ZmodemState);
        {$ENDIF}
        {$IFDEF DemoKermit}
        State := Ord(KermitPtr(PP)^.KermitState);
        {$ENDIF}
        {$IFDEF DemoBPlus}
        State := Ord(BPlusPtr(PP)^.BPlusState);
        {$ENDIF}

        NewTimer(RepeatTimer, 36);
        FastWrite('  Protocol:        ' +
                  ProtocolTypeString[GetProtocol(PP)], 11, 21, WinAttr);
        FastWrite('  Filename:        ' +
                  GetFilename(PP), 12, 21, WinAttr);
        FastWrite('  Protocol State:  ' +
                  LeftPad(Long2Str(State),2), 13, 21, WinAttr);
        FastWrite('  Elapsed Time:    ' +
                  FormatMinSec(Tics2Secs(GetElapsedTics(PP))), 14, 21, WinAttr);
        FastWrite('  Transferred:     ' +
                  LeftPad(Long2Str(GetBytesTransferred(PP)),8), 15, 21, WinAttr);
        FastWrite('  Remaining:       ' +
                  LeftPad(Long2Str(GetBytesRemaining(PP)),8), 16, 21, WinAttr);

        {Calculate and display throughput}
        if GetElapsedTics(PP) > 0 then begin
          R := GetBytesTransferred(PP) - GetInitialFilePos(PP);
          ActualCPS := R / (GetElapsedTics(PP) / 18.2);
        end else
          ActualCPS := 0.0;
...
procedure UnloadBack;
  {-Try to unload the resident copy of BACK}
var
  P : IfcPtr;
  Regs : IntRegisters;
begin
  ...
  if P = nil then
    WriteLn('Back isn''t loaded.')
  else if (P^.CmdEntryPtr <> nil) then begin
    RestoreAllVectors;
    Regs.AH := 1;
    EmulateInt(Regs, P^.CmdEntryPtr);
    if Regs.AL = 1 then
      WriteLn('Back Unloaded')
    else
      WriteLn('Can''t unload Back');
  end;
  Halt;
end;
...
begin
  ...
  if ModuleInstalled(ModuleName) then begin
    WriteLn('Back already installed');
    Halt;
  end;
  ...
  if DefinePop(HotKey1, MainPop, @PopStack[SizeOf(PopStack)], True) and
    ...
    HighVideo;
    WriteLn(^M^J,PgmId, ' - A demonstration of TSR background protocol');
    WriteLn('Copyright (c) 1992 by TurboPower Software');
    LowVideo;
    WriteLn;
    WriteLn('Back loaded, press Ctrl-RightShift-P to start protocol.');
